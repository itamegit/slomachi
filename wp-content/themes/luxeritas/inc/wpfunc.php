...
function thk_intersection_observer_replace( $content, $script = false ) {
	...
	if( stripos( $content, 'data-src=' ) === false  ) {
		if( stripos( $content, 'class=' ) !== false ) {
			$content = str_replace( 'src=', 'src="' . TDEL . '/images/trans.png" data-src=', $content );
			$content = preg_replace( '/(class=[\'|\"]*)/', '$1lazy ', $content );
		}
		else {
			$content = str_replace( 'src=', 'src="' . TDEL . '/images/trans.png" class="lazy" data-src=', $content );
		}
...
function thk_create_toc( $contents, $contents_replace = false ) {
	...
	if( stripos( $contents, '<h2' ) !== false || stripos( $contents, '<h3' ) !== false ) {
		...
		if( preg_match_all( '/<h([2-' . $h . ']{1})[^>]*?>(.*?)<\/h\1>/im', $contents, $matches, PREG_SET_ORDER ) ) {
			...
			foreach( (array)$matches as $m ) {
				...
				if( $contents_replace === true ) {
					...
					preg_match( '/<h[2-6]{1} id=["\'\s]*([^>"\'\s]+)["\'\s]*[^>]*?>' . $quote_text . '/im', $contents, $already );

					if( !isset( $already[1] ) ) {
						$contents = preg_replace( '/<h([2-6]{1})([^>]*?)>' . $quote_text . '/im', '<h$1 id="' . $target . '"$2>' . $temp, $contents, 1 );
					}
					else {
			...
			$contents = preg_replace( '/<(h[2-6]{1}[^>]*?)>(.+?)' . '#\*\t[0-9]+?\t\*#' . '(.*?<\/h[2-6]>)/im', '<$1>$2$3', $contents );
		}
	}
