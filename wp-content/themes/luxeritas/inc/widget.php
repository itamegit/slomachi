...
<div class="comment_excerpt"><i class="<?php echo $far, $fa_comment; ?>"></i><?php echo $excerpt; ?></div>
...
class thk_follow_button extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		elseif( $luxe['site_name_type'] === 'Organization' && isset( $luxe['organization_type'] ) ) {
 ...
 * script elimination
 *---------------------------------------------------------------------------*/
if( function_exists( 'thk_elim_script' ) === false ):
function thk_elim_script( $ret ) {
	$analytics = array(
		'add-analytics-head.php' => array(),
		'add-analytics.php'      => array(),
	);
	$f = TDEL === SDEL ? TPATH . DSEP : SPATH . DSEP;

	foreach( $analytics as $key => $val ) {
		ob_start();
		if( file_exists( $f . $key ) === true ) {
			require( $f . $key );
		}
		$analytics[$key][0] = trim( ob_get_clean() );
		$analytics[$key][1] = '<!--## ' . $key . ' ##-->';

		$ret = str_replace( $analytics[$key][0], $analytics[$key][1], $ret );
	}
	$ret = preg_replace( '/<script[^>]*?>.*?<\/script>\s*/ism', '', $ret );

	foreach( $analytics as $key => $val ) {
		$ret = str_replace( $analytics[$key][1], $analytics[$key][0], $ret );
	}
...
add_action( 'template_redirect', function() {
	global $luxe, $wp_registered_widgets;
	if( isset( $luxe['rc'] ) && !isset( $luxe['amp'] ) ) {
		foreach( (array)$wp_registered_widgets as $k => $v ) {
			if( 'thk_adsense_widget' === $v['callback'][0]->id_base ) {
...
add_action( 'thk_footer', function( $foot ) {
	global $luxe;
	if( isset( $luxe['rc'] ) && !isset( $luxe['amp'] ) ) {
		$foot = str_replace( '<!--/#wp-footer-->',
			'<script src="' . THK_HOME_URL . WPINC . '/js/jquery/jquery.js" defer></script>' .
			'<script src="' . TDEL . '/js/luxe.min.js" defer></script>' .
			'<!--/#wp-footer-->'
		, $foot );
	}
	return $foot;
...
if( function_exists( 'thk_remote_check' ) === false ):
function thk_remote_check() {
	require_once( INC . 'sections' . DSEP . 'rccheck.php' );
	if( thk_is_remote() === true ) return true;
	return false;
}
endif;
